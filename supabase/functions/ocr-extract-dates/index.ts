...
const SUBMISSION_PATTERNS = [
  ...
  // 躉購合約專用：「貴公司於114年4月28日提出申請辦理簽約手續」
  { pattern: /貴公司於\s*(?:中華)?民?國?\s*(\d{2,3})\s*年\s*(\d{1,2})\s*月\s*(\d{1,2})\s*日\s*提出申請/, source: '貴公司於...提出申請' },
  { pattern: /[臺台]端於\s*(?:中華)?民?國?\s*(\d{2,3})\s*年\s*(\d{1,2})\s*月\s*(\d{1,2})\s*日\s*提出申請/, source: '臺端於...提出申請' },
];
...
■ 躉購合約/躉售合約（台電發文）：
  - 可提取：送件日、核發日
  - 送件日特殊格式：「貴公司於 X年X月X日 提出申請辦理簽約手續」
  - 送件日位置：通常在「說明：」段落中
  - 注意：躉購合約的送件日是申請簽約的日期，不是發文日期
