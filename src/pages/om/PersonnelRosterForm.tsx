...
function createEmptyEntry(index: number): PersonnelEntry {
  return {
    seq: index + 1,
    role: index < DEFAULT_ROLES.length ? DEFAULT_ROLES[index] : '工作人員',
    name: '',
    gender: '',
    birthDate: '',
    bloodType: '',
...
const INITIAL_DATA: PersonnelRosterData = {
  ...
  contractor: '',
  personnel: Array.from({ length: 5 }, (_, i) => createEmptyEntry(i)),
};
...
export default function PersonnelRosterForm() {
  ...
  const addPerson = useCallback(() => {
    setData((prev) => {
      if (prev.personnel.length >= 20) {
        toast.error('最多 20 位人員');
        return prev;
      }
      return {
        ...prev,
        personnel: [...prev.personnel, createEmptyEntry(prev.personnel.length)],
      };
    });
  }, []);
  ...
  const removePerson = useCallback((index: number) => {
    setData((prev) => {
      const personnel = prev.personnel.filter((_, i) => i !== index).map((p, i) => ({ ...p, seq: i + 1 }));
      return { ...prev, personnel };
    });
  }, []);
  ...
  const handleExport = async () => {
    ...
    setIsExporting(true);
    try {
      await generatePersonnelRosterPdf(data);
      toast.success('人員名冊 PDF 已產生');
    } catch (err) {
      console.error(err);
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="space-y-1.5">
              <Label>施工日期</Label>
              <Input type="date" value={data.constructionDate} onChange={(e) => update('constructionDate', e.target.value)} />
            </div>
            <div className="space-y-1.5">
              <Label>承攬商</Label>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <Button variant="outline" size="sm" onClick={addPerson} disabled={data.personnel.length >= 20}>
            <Plus className="w-4 h-4 mr-1" /> 新增人員
          </Button>
        </CardHeader>
        <CardContent>
                  ...
                  <tr key={i} className="border-b last:border-b-0">
                    ...
                    <td className="py-1.5 px-1">
                      <Input
                        ...
                        onChange={(e) => updatePerson(i, 'emergencyContact', e.target.value)}
                        placeholder="聯絡人／電話"
                      />
                    </td>
                    <td className="py-1.5 px-1">
                      <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => removePerson(i)}>
          ...
          <p className="text-xs text-muted-foreground mt-3">
            ※ 承攬雇主之施作人員（含臨時聘僱作業人員）應投保相關保險並應將保單證明附上
          </p>
        </CardContent>
      </Card>
    </div>
